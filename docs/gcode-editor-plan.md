...
| **Scrub bar / step-through** tied to machine state | **Build — new, offline first — offline half DONE 2026-09-22, conditional-execution-aware 2026-09-22** | The "step through a file" feature from earlier; live single-step (sending real commands) is an explicitly separate, later phase — this plugin deliberately has no `sendCode` today (`docs/tasks/13-simulation-and-tail.md`'s own stop point). Offline scrub bar + step buttons + derived-state readout shipped (`GcodeStepperPanel.vue`, `GcodeEditor.vue`, `state.ts`'s new X/Y/E tracking, `dwc-gcode-editor` v0.8.0's `gcodeCurrentLine()`) — originally a flat top-to-bottom walk. Now walks REAL execution order instead: `dwc-gcode-core` 1.5.x's `walkExecution` (an RRF-faithful `if`/`elif`/`else`/`while`/`break`/`continue`/`abort` interpreter over its existing block tree) decides which lines actually run, and — since this plugin has no live machine — pauses on any object-model path a condition references that it can't know offline, prompting the user for a hypothetical value (`model/gcode/simulatedValues.ts`) that then affects every subsequent line, same as it would on real hardware. Live single-stepping against real hardware remains a separate, unstarted, higher-risk follow-on needing its own safety design. |
