...
| **Scrub bar / step-through** tied to machine state | **Build — new, offline first — offline half DONE 2026-09-22, conditional-execution-aware 2026-09-22, known-state + persistence 2026-09-22, M291 message boxes (incl. S4 choice) 2026-09-22, objectModelVersion wired up 2026-09-22** | The "step through a file" feature from earlier; live single-step (sending real commands) is an explicitly separate, later phase — this plugin deliberately has no `sendCode` today (`docs/tasks/13-simulation-and-tail.md`'s own stop point). Offline scrub bar + step buttons + derived-state readout shipped (`GcodeStepperPanel.vue`, `GcodeEditor.vue`, `state.ts`'s new X/Y/E tracking, `dwc-gcode-editor` v0.8.0's `gcodeCurrentLine()`) — originally a flat top-to-bottom walk. Now walks REAL execution order instead: `dwc-gcode-core` 1.9.x's `walkExecution` (an RRF-faithful `if`/`elif`/`else`/`while`/`break`/`continue`/`abort` interpreter, block-scoped `var`, opt-in object-model schema validation, over its existing block tree) decides which lines actually run, and — since this plugin has no live machine — pauses on any object-model path a condition references that it can't already answer, prompting the user for a hypothetical value (`model/gcode/simulatedValues.ts`) that then affects every subsequent line, same as it would on real hardware. `executionIndex.ts` answers a handful of paths itself before asking (`move.axes[0..2].homed`/`userPosition` and `state.currentTool`, all derived live from `state.ts`'s own tracking via `walkExecution`'s `onStep` hook — NOT `machinePosition`, which needs workplace/tool offsets this tracker doesn't have) — most conditions still need a real answer from the user, which is now persisted per file (`localStorage`, survives closing and reopening it). When the connected board's own firmware version has real schema data (`machineSnapshot.ts`'s `trackedObjectModelVersion` — never an untracked/build-suffixed version, which would make every condition a hard error instead), a typo'd or nonexistent object-model path is flagged as an error instead of pausing to ask for a value that could never be right. A blocking `M291` message box now pauses the walk too and shows a real prompt matching the box's own buttons — OK / OK+Cancel / a bounds-checked value field (integer/float/string) / a button per choice for `S4` — with `input`/`result` readable by later lines exactly as they would be on real hardware; cancelling an OK+Cancel box aborts the walk by default, matching RRF's own default. Both simulated values and message-box answers are individually removable (chips with a close icon, the choice ones showing the chosen option's own text) as well as clearable all at once. Live single-stepping against real hardware remains a separate, unstarted, higher-risk follow-on needing its own safety design. |
