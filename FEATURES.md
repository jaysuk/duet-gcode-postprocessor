...
| H9 | ✅ **Tool renumbering** - remap T0 to T2 without mangling comments and `M568 P0` | Done — `model/steps/toolRenumber.ts`. Rewrites bare `T<n>` and the tool-number parameter of `M563`/`M567`/`M568`/`M116`, and of `G10` only in its tool-settings form (never `G10 L2`/`L20`, where `P` is a workplace coordinate system) — decided by the shared `g10Form` (`model/gcode/toolTemperature.ts`), which follows RRF's own `case 10` dispatch; verified command-by-command against the RRF G-code dictionary; deliberately leaves `M106`/`M107` (fan index) and `M585` (Z probe number) alone |
| H10 | ✅ **Z-hop injection** on travels above a length threshold | Done — `model/steps/zHop.ts`, sharing travel detection with H11 via `model/steps/travel.ts`. Skips a travel with an existing hop, and the whole rest of a file using firmware retraction (a bare `G10`, or `G11` — a `G10` setting a temperature or offset does not count, per the shared `g10Form`) |
